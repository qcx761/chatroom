            ...
            if (events[i].events & EPOLLIN) {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        // JSON解析失败，尝试寻找下一个可能的合法长度字段
                        bool found_next = false;
                        for (size_t i = 1; i + 4 <= buffer.size(); ++i) {
                            uint32_t next_len_net;
                            memcpy(&next_len_net, buffer.data() + i, 4);
                            uint32_t next_len = ntohl(next_len_net);
                            if (next_len > 0 && next_len < 10 * 1024 * 1024 && i + 4 + next_len <= buffer.size()) {
                                cout << "JSON解析失败 fd=" << fd << ", 跳过 " << i << " 字节" << endl;
                                buffer.erase(0, i);
                                found_next = true;
                                break;
                            }
